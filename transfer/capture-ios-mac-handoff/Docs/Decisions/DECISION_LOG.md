...
| Decision | **No.** Keep `CAPTURE_IMPLEMENTATION_HANDOFF.md` (session), `PROJECT_STATE.md` (executive), and **EKP** (full knowledge) separate. |
...
## DECISION-0012: Rejection of ML-derived Semantic Mattes in Evidence Pipeline

- **Date:** 2026-07-24
- **Status:** APPROVED
- **Context:** Consideration of Apple AVFoundation portrait/semantic matte attachments during capture.
- **Related:** V2-001 (same decision; this block is the canonical narrative form)

### Question
Should ML-derived semantic segmentation mattes be stored as primary evidence artifacts in the capture container?

### Decision
**No.** ML-derived semantic mattes are prohibited from serving as primary ground-truth evidence. They may only exist as non-authoritative presentation metadata if explicitly flagged.

### Rationale
1. **Determinism:** ML inference outputs vary across OS updates and SoC generations for identical sensor frames.
2. **Chain of Custody:** Evidence boundaries require raw hardware-level sensor telemetry, not post-processed probabilistic models.

### Future Review Trigger
IR-0008 or when hardware-bound deterministic segmentation models become standard.

---

## DECISION-0013: Five Core Memory Artifacts (Canonical)

- **Date:** 2026-07-25
- **Status:** APPROVED
- **Context:** Handoff documents were collapsing session restore, executive status, ecosystem export, and rationale into one blob.

### Question
Should Capture keep a single handoff document for all memory jobs?

### Decision
**No.** Use five single-responsibility artifacts: `PROJECT_STATE.md`, `CAPTURE_IMPLEMENTATION_HANDOFF.md`, `REPOSITORY_MEMORY.md`, `Docs/Decisions/DECISION_LOG.md`, and the Elektron Knowledge Package (EKP).

### Rationale
1. Different audiences and refresh cadences.
2. Prevents bloated, noisy, contradictory “god documents.”
3. Enables EMKP aggregation later without rewriting session memory.
