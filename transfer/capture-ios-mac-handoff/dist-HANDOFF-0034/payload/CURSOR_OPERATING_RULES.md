...
| Supersedes | 0.1.2 operating rules |
...
Until all nested sub-gates pass, work remains `IMPLEMENTED_PENDING_COMPLETION_ARTIFACTS`.

```text
1–5  implementation + tests + evidence + changelog + change record
6    HANDOFF_METADATA_COMMITTED     (make handoff-prepare → commit)
7–12 source archive + bundle + inventory + digests + verify + restoration
     (make handoff-package on clean HEAD → dist/handoff-<id>/)
= IMPLEMENTATION_COMPLETE
```

```bash
make handoff-prepare    # Stage 1 — tracked metadata
git add … && git commit
make handoff-package    # Stage 2 — clean HEAD → untracked dist/
```

Single-shot `make handoff` is retired. Detailed records: `Docs/Changes/CHANGE-XXXX.md`.
History: `Docs/Handoffs/HANDOFF_HISTORY.md`.
