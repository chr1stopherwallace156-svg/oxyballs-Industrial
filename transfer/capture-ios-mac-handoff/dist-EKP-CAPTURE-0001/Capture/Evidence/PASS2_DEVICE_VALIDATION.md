...
| 8. Share .edts-pkg | Share Sheet / AirDrop presents the **`.edts-pkg` file** (not the package directory) |
| 9. Save to Files | Document export picker can save the `.edts-pkg` |
| 10. Export as ZIP copy | Diagnostic `.zip` appears; SHA-256 of zip file **equals** `.edts-pkg` file; canonical `.edts-pkg` still present |
| 11. Artifact check | On-disk `payload/artifact_original.jpg` SHA-256 **equals** frozen review SHA (unchanged by share/ZIP copy) |
| 12. Interrupt (optional) | Background/interrupt → Interrupted → Resume → previewing |
...
- Share prepare failure if path points at package **directory** instead of `.edts-pkg` file (should be fixed by P2-004)
...
5. Confirmation Share/AirDrop or Files saved `.edts-pkg` successfully
6. If used: `shasum -a 256` of diagnostic `.zip` equals `.edts-pkg`
7. Pass/fail for each checklist row
